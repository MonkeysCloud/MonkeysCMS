...
(function(window) {
    ...
    const CmsFields = {
        ...
        init() {
            ...
            document.querySelectorAll('[data-field-id]').forEach(el => {
                const widgetType = el.className.match(/field-(\w+)/)?.[1];
        ...
        init(fieldId) {
            ...
            if (counter && maxLength) {
                const update = () => {
                    ...
                    counter.classList.remove('field-textarea__counter--warning', 'field-textarea__counter--error');
                    if (remaining < 0) {
        ...
        initImage(fieldId) {
            ...
            const updatePreview = (url) => {
                if (url) {
                    ...
                    preview.innerHTML = '<div class="field-image__placeholder">No image selected</div>';
            ...
            const renderGrid = () => {
                ...
                grid.querySelectorAll('[data-action="remove"]').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        const index = parseInt(e.target.closest('.field-gallery__item').dataset.index);
    ...
    const CmsEntityReference = {
        init(fieldId, apiUrl) {
            ...
            const hiddenInput = document.getElementById(fieldId);
            const searchInput = wrapper.querySelector('.field-entity-reference__input');
            const dropdown = document.getElementById(fieldId + '_dropdown');
            const selected = wrapper.querySelector('.field-entity-reference__selected');
            const multiple = wrapper.dataset.multiple === 'true';
            
            let values = [];
            try {
                values = multiple ? JSON.parse(hiddenInput.value) : (hiddenInput.value ? [hiddenInput.value] : []);
            } catch (e) {}
            ...
            const renderSelected = () => {
                selected.innerHTML = values.map(v => `
                ...
                selected.querySelectorAll('[data-action="remove"]').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        const value = e.target.closest('.field-entity-reference__item').dataset.value;
            ...
            searchInput?.addEventListener('input', () => {
                ...
                debounceTimer = setTimeout(async () => {
                    ...
                    try {
                        ...
                        `).join('') || '<div class="field-entity-reference__empty">No results found</div>';
                        
                        dropdown.classList.add('field-entity-reference__dropdown--open');
                        
                        dropdown.querySelectorAll('.field-entity-reference__option').forEach(opt => {
    ...
    const CmsTaxonomy = {
        ...
        initTags(fieldId, apiUrl) {
            ...
            const renderTags = () => {
                ...
                tagsContainer.querySelectorAll('.field-taxonomy__tag-remove').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        const tag = e.target.closest('.field-taxonomy__tag').dataset.value;
            ...
            tagInput?.addEventListener('input', () => {
                ...
                debounceTimer = setTimeout(async () => {
                    ...
                    try {
                        ...
                        suggestions.querySelectorAll('.field-taxonomy__suggestion').forEach(opt => {
                            opt.addEventListener('click', () => {
    ...
    const CmsRepeater = {
        init(fieldId) {
            ...
            let itemCount = itemsContainer.querySelectorAll('.field-repeater__item').length;
            ...
            const addItem = () => {
                ...
                const html = template.innerHTML.replace(/__INDEX__/g, itemCount.toString());
            ...
            const removeItem = (item) => {
                if (confirmDelete && !confirm('Are you sure you want to remove this item?')) {
                    return;
                }
            ...
            itemsContainer.querySelectorAll('.field-repeater__item').forEach(initItem);
        ...
        initDragDrop(container, onUpdate) {
            ...
            function getDragAfterElement(container, y) {
                const draggableElements = [...container.querySelectorAll('.field-repeater__item:not(.dragging)')];
            ...
            toolbar?.querySelectorAll('.field-markdown__btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    if (action === 'toggle-preview') {
                        ...
                        if (preview.style.display !== 'none' && typeof marked !== 'undefined') {
    ...
    const CmsForm = {
            ...
            form.addEventListener('submit', async (e) => {
                if (form.dataset.ajax === 'true') {
                    ...
                    try {
                        ...
                        this.showErrors(form, { _form: ['An error occurred. Please try again.'] });
                    } finally {
        ...
        showErrors(form, errors) {
            ...
            if (errors._form) {
                ...
                if (formErrors) {
                    formErrors.innerHTML = errors._form.map(e => `<div>${e}</div>`).join('');
            ...
            Object.entries(errors).forEach(([field, messages]) => {
                ...
                const widget = form.querySelector(`[data-field="${field}"]`)?.closest('.field-widget');
                if (widget) {
                    widget.classList.add('field-widget--has-error');
                    const errorsContainer = widget.querySelector('.field-widget__errors');
                    if (errorsContainer) {
                        errorsContainer.innerHTML = messages.map(m => 
                            `<div class="field-widget__error">${m}</div>`
                        ).join('');
                    }
                }
            });

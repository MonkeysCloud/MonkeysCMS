...
(function() {
    ...
    async function loadThemes() {
        showLoading(true);
        try {
            const response = await adminApi.get('/api/admin/themes');
            allThemes = response.themes || [];
            activeTheme = response.active || '';
            renderThemes();
            populateParentSelect();
        } catch (error) {
            showToast('Failed to load themes: ' + (error.message || 'Unknown error'), 'danger');
        }
        showLoading(false);
    }
    ...
    window.activateTheme = async function(name) {
        ...
        try {
            await adminApi.post('/api/admin/themes/' + encodeURIComponent(name) + '/activate');
            showToast(`Theme "${name}" activated successfully`, 'success');
            await loadThemes();
        } catch (error) {
            showToast('Failed to activate theme: ' + (error.message || 'Unknown error'), 'danger');
        }
    };

    window.validateTheme = async function(name) {
        try {
            const result = await adminApi.get('/api/admin/themes/' + encodeURIComponent(name) + '/validate');
            if (result.valid) {
                showToast(`Theme "${name}" is valid ✓`, 'success');
            } else {
                showToast(`Theme "${name}" has ${result.errors.length} validation error(s):\n${result.errors.join('\n')}`, 'warning');
            }
        } catch (error) {
            showToast('Validation failed: ' + (error.message || 'Unknown error'), 'danger');
        }
    };

    window.clearThemeCache = async function() {
        try {
            await adminApi.post('/api/admin/themes/cache/clear');
            showToast('Theme cache cleared', 'success');
            await loadThemes();
        } catch (error) {
    ...
    window.viewThemeDetails = async function(name) {
        try {
            const theme = await adminApi.get('/api/admin/themes/' + encodeURIComponent(name));
            ...
            showToast('Failed to load theme details: ' + (error.message || 'Unknown error'), 'danger');
        }
    };
    ...
    window.handleCreateTheme = async function(e) {
        ...
        try {
            await adminApi.post('/api/admin/themes', data);
            showToast(`Theme "${data.name}" created successfully`, 'success');
            closeCreateModal();
            await loadThemes();
        } catch (error) {
            showToast('Failed to create theme: ' + (error.message || 'Unknown error'), 'danger');
        }
    };

...
<div x-data="mediaUpload()" class="max-w-4xl mx-auto">
    ...
    <div 
        ...
        :class="{'border-blue-500 bg-blue-50 ring-4 ring-blue-100': isDragging, 'border-gray-300 hover:border-gray-400 bg-white': !isDragging}"
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0 space-y-3">
                            ...
                            <!-- Actions -->
                             <div class="flex items-center gap-2">
                                <template x-if="file.status === 'pending' || file.status === 'error'">
                                    <button @click="startUpload(file.id)" class="text-xs font-medium text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1.5 rounded-md transition-colors">
                                        Upload
                                    </button>
                                </template>
                                <template x-if="file.status === 'uploading'">
                                     <button @click="cancelUpload(file.id)" class="text-xs font-medium text-red-500 hover:text-red-700 bg-red-50 px-3 py-1.5 rounded-md transition-colors">Cancel</button>
                                </template>
                                <template x-if="file.status === 'pending'">
                                    <button @click="cancelUpload(file.id)" class="text-gray-400 hover:text-gray-600">
                        ...
                        <div x-show="file.status === 'pending' || file.status === 'error'" class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
                            ...
                            <label class="block" :class="{'sm:col-span-2': !file.isImage}">

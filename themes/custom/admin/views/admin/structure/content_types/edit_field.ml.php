...
@section('content')
    ...
    <x-ui.card class="max-w-3xl">
        ...
        <form method="post" action="{{ $action }}">
            ...
            <div class="mb-4">
                <label for="settings_json" class="block text-sm font-medium text-gray-700 mb-1">Configuration (JSON)</label>
                <textarea name="settings_json" id="settings_json" rows="6" font-family="monospace"
                    class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm px-3 py-2 font-mono text-xs">{{ isset($field['settings']) ? json_encode($field['settings'], JSON_PRETTY_PRINT) : '{}' }}</textarea>
                <p class="mt-1 text-xs text-gray-500">Advanced settings for the field widget.</p>
            </div>
